...
accuracy each trainable parameter buys, we compare multiple adaptation
...
of patch and word tokens through the \textbf{Sinkhorn} distance.  These results are
...
tabulated as GPU memory, training time and trainable parameters are seldom
...
total, projecting into a shared 512-dimensional space and very backbone
...
joint model: all of them inherit a \texttt{predict()} that scores the
two by cosine similarity.  Three methods provide their own decision rule as follows: Tip-Adapter (cache logits added to the zero-shot ones), Optimal Transport (a transport cost, minimised rather than maximised) and the linear probe (a fitted classifier).  A single evaluation loop consumes them all, which is what makes the accuracy, memory and timing columns comparable.


\subsection{Baselines}\label{sec:baselines}

\textbf{Zero-shot CLIP} encodes each class name with a template (\eg\``a satellite image of \{class\}'') and predicts the class with the highest cosine similarity. \textbf{Prompt ensembling}~\cite{radford2021learning} averages the prototypes of seven
templates (\eg\ ``an aerial view of \{\}'') at no parameter cost; the \textbf{linear probe} fits an $\ell_2$-regularised logistic regression (\texttt{scikit-learn}) on the frozen image features, with $C(D+1)$ parameters for $C$ classes and $D = 512$.
...
The na\"ive multiple inheritance fails on the Method Resolution Order (MRO):
...
An earlier full-shot run applied CoOp's (SGD at $2\times10^{-3}$) to both, and the adapter,
initialised near zero, barely moved: the joint model lost to the plain adapter on all three datasets, by 7.6 points on Flowers102. We then decided to expose the two modules as separate parameter groups under one AdamW optimiser and train both at the adapter's $10^{-3}$; the split keeps a per-module learning-rate ratio available, which we did not need.

\subsection{Tip-Adapter and Tip-Adapter-F}\label{sec:tip}

Tip-Adapter~\cite{zhang2022tip} builds a training-free cache from the support set, with frozen image features $\mathbf{F}_{\mathrm{train}} \in \mathbb{R}^{N \times D}$ as keys and
...
$10^{-4}$.  It is the only method we found that changes the computation inside the
...
peaks separate them as CLIP-Adapter stays at 1{,}339--1{,}420\,MB on all
...
visual blocks, while CoOp grows with the number of classes (1{,}424, 2{,}138
...
joint model follows it within 2\,MB. It appears as though few trainable parameters do not
imply a light training step.

\subsection{Analysis of Optimal Transport Failure}\label{sec:ot-analysis}

We seem to have identified three structural reasons for the failure:
...
58.6\,\% (77.2\,\% at $2\times10^{-5}$); from $2\times10^{-4}$ on,
...
keep the $2\times10^{-3}$ of the original.  With 8{,}192 parameters
...
Quadrupling the context from $M{=}4$ to $M{=}16$ buys 0.32 points so it is safe to assume that at this support-set size the prompt's expressiveness is not the bottleneck.
...
rates.  This supports the original study, which finds unified context
...
0.23-point $\alpha$ sweep carries no signal, likely because the
...
5.4--5.7 points over CoOp, which suggests that their gain comes less
from tighter clusters than from bringing image features closer to the
text prototypes as well. LoRA is the only method that visibly separates the classes (0.245), yet it is no more accurate than the adapter: separability of the image features and
...
variant) on three benchmarks with a common frozen ViT-B/32 backbone, and four conclusions stand out. 
\textbf{Labels impact more than parameters}: once 16 images per class (10 on Flowers102) are available, the parameter count predicts accuracy poorly.  On EuroSAT the training-free Tip-Adapter and a 5{,}130-parameter linear probe close 70\,\% and 82\,\% of the gap between
zero-shot CLIP and the best method, and within CoOp the shot count moves
accuracy far more than context length or type. 
\textbf{Extreme parameter efficiency is achievable where the text side matters}: on DTD
and Flowers102, CoOp's 16 context vectors (0.005\,\% of the backbone) close 90\,\% and 76\,\% of that gap and beat the linear probe with fewer parameters.  
\textbf{Joint text--vision adaptation pproduces marginal gains}: the combined model lands within 0.33 points of the better half on all three datasets ($+0.33$, $-0.16$, $-0.23$), suggesting overlapping contributions, and reaching even that parity required the adapter's
optimiser recipe rather than CoOp's.  \textbf{Token-level Optimal Transport fails without careful engineering}, for the structural reasons of Section~\ref{sec:ot-analysis}.  Two observations qualify these results: Tip-Adapter-F is the most accurate method on two datasets out of three, but its parameter count grows with the number of classes; and few
...
stays near 1.4\,GB. In practice, we start with prompt ensembling, which is
free; with labels, we try Tip-Adapter and a linear probe first; move to CoOp
...
for the CoOp ablations), as training was lengthy and heavy on that alone; the only spread we measure, 0.06 points between two runs of one adapter configuration, is smaller than the effects we discuss, but such small differences are not really meaningful, and
wall-clock times are even noisier. The \textbf{ablations follow different protocols} from the main grid and from each other, both on
...
\texttt{@torch.no\_grad()} so that gradients reach the context. Wrote the
...
The following elements of this project were assisted by the respective cited tool:
\begin{itemize}
    \item \textbf{Graphic visualization}: Aid in creating quality graphs and tables from obtained csv data to better display the results and metrics. [Claude]
    \item \textbf{GPU-specific code refactoring}: Given that the code was run on a local machine and not on Google Colab's framework, the code was refactored a few times to introduce efficiency-specific optimization for its execution on GPU-specific modules (mainly utilizing Cuda-Conda) (NVIDIA RTX 3060 TI - 6GB). [Claude]
    \item \textbf{Different code modules connection}: As each member wrote their part of the code of this repository, we were aided in integrating each module with the others' to save time and focus more thoroughly on the theoretical part. [Claude]
    \item \textbf{Grammar overview}: Given the paper-like tone of the report, we asked for grammatical input throughout the development to ensure no spelling or grammar mistakes were made and to rewrite some content in a more formal way. [Claude]
    \item \textbf{Reference Aid}: Aid in looking up references for papers (mainly exploring current SOTA) including topics regarding this report and project. [GPT]
\end{itemize}
